...
/*
 * turned this into include/header file
 * - having defined some token replacements,
 * see pocketffts_single.c and pocketffts_double.c.
 * all 'double' got replaced by 'floatT' ..
 * thus, it's sort of templating - utilizing the C preprocessor,
 * to get a single precision 'float' ffts implementation
...
static void my_sincosm1pi (floatT a, floatT *restrict res)
  ...
  floatT r =     -1.0369917389758117e-4;
  r = fmaT(r, s,  1.9294935641298806e-3);
  r = fmaT(r, s, -2.5806887942825395e-2);
  r = fmaT(r, s,  2.3533063028328211e-1);
  r = fmaT(r, s, -1.3352627688538006e+0);
  r = fmaT(r, s,  4.0587121264167623e+0);
  r = fmaT(r, s, -4.9348022005446790e+0);
  floatT c = r*s;
  /* Approximate sin(pi*x) for x in [-0.25,0.25] */
  r =             4.6151442520157035e-4;
  r = fmaT(r, s, -7.3700183130883555e-3);
  r = fmaT(r, s,  8.2145868949323936e-2);
  r = fmaT(r, s, -5.9926452893214921e-1);
  r = fmaT(r, s,  2.5501640398732688e+0);
  r = fmaT(r, s, -5.1677127800499516e+0);
  s = s * a;
  r = r * s;
  s = fmaT(a, 3.1415926535897931e+0, r);
  res[0] = c;
  res[1] = s;
  }

NOINLINE static void calc_first_octant(size_t den, floatT * restrict res)
  {
  size_t n = (den+4)>>3;
  if (n==0) return;
  res[0]=1.; res[1]=0.;
  ...
  while(start<n)
    ...
    for (size_t i=1; i<end; ++i)
      {
      floatT csx[2]={res[2*i], res[2*i+1]};
...
NOINLINE static void calc_first_quadrant(size_t n, floatT * restrict res)
  {
  floatT * restrict p = res+n;
...
NOINLINE static void calc_first_half(size_t n, floatT * restrict res)
  {
  int ndone=(n+1)>>1;
  floatT * p = res+n-1;
...
NOINLINE static void fill_first_quadrant(size_t n, floatT * restrict res)
  {
  const floatT hsqt2 = 0.707106781186547524400844362104849;
  size_t quart = n>>2;
  if ((n&7)==0)
...
NOINLINE static void fill_first_half(size_t n, floatT * restrict res)
  {
  size_t half = n>>1;
  if ((n&3)==0)
...
NOINLINE static void fill_second_half(size_t n, floatT * restrict res)
  {
  if ((n&1)==0)
...
NOINLINE static void sincos_2pibyn_half(size_t n, floatT * restrict res)
  {
  if ((n&3)==0)
    {
...
NOINLINE static void sincos_2pibyn(size_t n, floatT * restrict res)
  {
...
NOINLINE static size_t largest_prime_factor (size_t n)
  ...
  // note: without double sqrt(), we get endless loop - for some n!
...
NOINLINE static floatT cost_guess (size_t n)
  {
  const floatT lfp=1.1; // penalty for non-hardcoded larger factors
  size_t ni=n;
  floatT result=0.;
  size_t tmp;
  while (((tmp=(n>>1))<<1)==n)
...
#define NFCT 25
typedef struct cfftsp_fctdata
  {
  size_t fct;
  cmplx *tw, *tws;
  } cfftsp_fctdata;

typedef struct cfftsp_plan_i
  {
  size_t length, nfct;
  cmplx *mem;
  cfftsp_fctdata fct[NFCT];
  } cfftsp_plan_i;
typedef struct cfftsp_plan_i * cfftsp_plan;
...
#define ROT90(a) { floatT tmp_=a.r; a.r=-a.i; a.i=tmp_; }
#define ROTM90(a) { floatT tmp_=-a.r; a.r=a.i; a.i=tmp_; }
...
/* a *= b */
#define MULPMSIGNCEQ(a,b) { floatT xtmp=a.r; a.r=b.r*a.r-sign*b.i*a.i; a.i=b.r*a.i+sign*b.i*xtmp; }
...
NOINLINE static void pass3b (size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  {
  const size_t cdim=3;
  const floatT tw1r=-0.5, tw1i= 0.86602540378443864676;
...
NOINLINE static void pass5b (size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  {
  const size_t cdim=5;
  const floatT tw1r= 0.3090169943749474241,
...
NOINLINE static void pass7(size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  {
  const size_t cdim=7;
  const floatT tw1r= 0.623489801858733530525,
...
NOINLINE static void pass11 (size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  const floatT tw1r =        0.8412535328311811688618,
...
NOINLINE WARN_UNUSED_RESULT static int pass_all(cfftsp_plan plan, cmplx c[], floatT fct,
  const int sign)
  {
...
static int cfftsp_forward(cfftsp_plan plan, floatT c[], floatT fct)
  { return pass_all(plan,(cmplx *)c, fct, -1); }

NOINLINE WARN_UNUSED_RESULT
static int cfftsp_backward(cfftsp_plan plan, floatT c[], floatT fct)
  { return pass_all(plan,(cmplx *)c, fct, 1); }

NOINLINE WARN_UNUSED_RESULT
static int cfftsp_factorize (cfftsp_plan plan)
  {
...
NOINLINE static size_t cfftsp_twsize (cfftsp_plan plan)
  {
...
NOINLINE WARN_UNUSED_RESULT static int cfftsp_comp_twiddle (cfftsp_plan plan)
  {
  size_t length=plan->length;
  floatT *twid = RALLOC(floatT, 2*length);
  if (!twid) return -1;
...
static cfftsp_plan make_cfftsp_plan (size_t length)
  {
  if (length==0) return NULL;
  cfftsp_plan plan = RALLOC(cfftsp_plan_i,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  for (size_t i=0; i<NFCT; ++i)
    plan->fct[i]=(cfftsp_fctdata){0,0,0};
  plan->mem=0;
  if (length==1) return plan;
  if (cfftsp_factorize(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=cfftsp_twsize(plan);
  plan->mem=RALLOC(cmplx,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (cfftsp_comp_twiddle(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }

static void destroy_cfftsp_plan (cfftsp_plan plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }

typedef struct rfftsp_fctdata
  {
  size_t fct;
  floatT *tw, *tws;
  } rfftsp_fctdata;

typedef struct rfftsp_plan_i
  {
  size_t length, nfct;
  floatT *mem;
  rfftsp_fctdata fct[NFCT];
  } rfftsp_plan_i;
typedef struct rfftsp_plan_i * rfftsp_plan;
...
NOINLINE static void radf2 (size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void radf3(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=3;
  static const floatT taur=-0.5, taui=0.86602540378443864676;

  for (size_t k=0; k<l1; k++)
    {
    floatT cr2=CC(0,k,1)+CC(0,k,2);
  ...
  for (size_t k=0; k<l1; k++)
    for (size_t i=2; i<ido; i+=2)
      ...
      floatT cr2=dr2+dr3; // c add
      floatT ci2=di2+di3;
      CH(i-1,0,k) = CC(i-1,k,0)+cr2; // c add
      CH(i  ,0,k) = CC(i  ,k,0)+ci2;
      floatT tr2 = CC(i-1,k,0)+taur*cr2; // c add
      floatT ti2 = CC(i  ,k,0)+taur*ci2;
      floatT tr3 = taui*(di2-di3);  // t3 = taui*i*(d3-d2)?
      floatT ti3 = taui*(dr3-dr2);
      PM(CH(i-1,2,k),CH(ic-1,1,k),tr2,tr3) // PM(i) = t2+t3
      PM(CH(i  ,2,k),CH(ic  ,1,k),ti3,ti2) // PM(ic) = conj(t2-t3)
      }
  }

NOINLINE static void radf4(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=4;
  static const floatT hsqt2=0.70710678118654752440;
  ...
  if ((ido&1)==0)
    for (size_t k=0; k<l1; k++)
      {
      floatT ti1=-hsqt2*(CC(ido-1,k,1)+CC(ido-1,k,3));
      floatT tr1= hsqt2*(CC(ido-1,k,1)-CC(ido-1,k,3));
  ...
  for (size_t k=0; k<l1; k++)
    for (size_t i=2; i<ido; i+=2)
      {
      size_t ic=ido-i;
      floatT ci2, ci3, ci4, cr2, cr3, cr4, ti1, ti2, ti3, ti4, tr1, tr2, tr3, tr4;
...
NOINLINE static void radf5(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=5;
  static const floatT tr11= 0.3090169943749474241, ti11=0.95105651629515357212,
                      tr12=-0.8090169943749474241, ti12=0.58778525229247312917;

  for (size_t k=0; k<l1; k++)
    {
    floatT cr2, cr3, ci4, ci5;
  ...
  for (size_t k=0; k<l1;++k)
    for (size_t i=2; i<ido; i+=2)
      {
      floatT ci2, di2, ci4, ci5, di3, di4, di5, ci3, cr2, cr3, dr2, dr3,
...
NOINLINE static void radfg(size_t ido, size_t ip, size_t l1,
  floatT * restrict cc, floatT * restrict ch, const floatT * restrict wa,
  const floatT * restrict csarr)
  {
  const size_t cdim=ip;
  size_t ipph=(ip+1)/2;
  size_t idl1 = ido*l1;
    ...
    for (size_t j=1, jc=ip-1; j<ipph; ++j,--jc)              // 114
      ...
      for (size_t k=0; k<l1; ++k)                            // 113
        ...
        for (size_t i=1; i<=ido-2; i+=2)                      // 112
          {
          floatT t1=C1(i,k,j ), t2=C1(i+1,k,j ),
                 t3=C1(i,k,jc), t4=C1(i+1,k,jc);
          floatT x1=wa[idij]*t1 + wa[idij+1]*t2,
  ...
  for (size_t j=1, jc=ip-1; j<ipph; ++j,--jc)                // 123
    for (size_t k=0; k<l1; ++k)                              // 122
      {
      floatT t1=C1(0,k,j), t2=C1(0,k,jc);
      C1(0,k,j ) = t1+t2;
      C1(0,k,jc) = t2-t1;
      }

//everything in C
//memset(ch,0,ip*l1*ido*sizeof(floatT));
  ...
  for (size_t l=1,lc=ip-1; l<ipph; ++l,--lc)                 // 127
    ...
    for (; j<ipph-3; j+=4,jc-=4)              // 126
      ...
      floatT ar1=csarr[2*iang], ai1=csarr[2*iang+1];
      iang+=l; if (iang>=ip) iang-=ip;
      floatT ar2=csarr[2*iang], ai2=csarr[2*iang+1];
      iang+=l; if (iang>=ip) iang-=ip;
      floatT ar3=csarr[2*iang], ai3=csarr[2*iang+1];
      iang+=l; if (iang>=ip) iang-=ip;
      floatT ar4=csarr[2*iang], ai4=csarr[2*iang+1];
    ...
    for (; j<ipph; ++j,--jc)              // 126
      {
      iang+=l; if (iang>=ip) iang-=ip;
      floatT ar=csarr[2*iang], ai=csarr[2*iang+1];
      for (size_t ik=0; ik<idl1; ++ik)                       // 125
        {
        CH2(ik,l ) += ar*C2(ik,j );
...
//memset(cc,0,ip*l1*ido*sizeof(floatT));
...
NOINLINE static void radb2(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void radb3(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=3;
  static const floatT taur=-0.5, taui=0.86602540378443864676;
  ...
  for (size_t k=0; k<l1; k++)
    {
    floatT tr2=2.*CC(ido-1,1,k);
    floatT cr2=CC(0,0,k)+taur*tr2;
    CH(0,k,0)=CC(0,0,k)+tr2;
    floatT ci3=2.*taui*CC(0,2,k);
  ...
  for (size_t k=0; k<l1; k++)
    for (size_t i=2; i<ido; i+=2)
      {
      size_t ic=ido-i;
      floatT tr2=CC(i-1,2,k)+CC(ic-1,1,k); // t2=CC(I) + conj(CC(ic))
      floatT ti2=CC(i  ,2,k)-CC(ic  ,1,k);
      floatT cr2=CC(i-1,0,k)+taur*tr2;     // c2=CC +taur*t2
      floatT ci2=CC(i  ,0,k)+taur*ti2;
      CH(i-1,k,0)=CC(i-1,0,k)+tr2;         // CH=CC+t2
      CH(i  ,k,0)=CC(i  ,0,k)+ti2;
      floatT cr3=taui*(CC(i-1,2,k)-CC(ic-1,1,k));// c3=taui*(CC(i)-conj(CC(ic)))
      floatT ci3=taui*(CC(i  ,2,k)+CC(ic  ,1,k));
      floatT di2, di3, dr2, dr3;
...
NOINLINE static void radb4(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=4;
  static const floatT sqrt2=1.41421356237309504880;

  for (size_t k=0; k<l1; k++)
    {
    floatT tr1, tr2;
    PM (tr2,tr1,CC(0,0,k),CC(ido-1,3,k))
    floatT tr3=2.*CC(ido-1,1,k);
    floatT tr4=2.*CC(0,2,k);
...
NOINLINE static void radb5(size_t ido, size_t l1, const floatT * restrict cc,
  floatT * restrict ch, const floatT * restrict wa)
  {
  const size_t cdim=5;
  static const floatT tr11= 0.3090169943749474241, ti11=0.95105651629515357212,
  ...
  for (size_t k=0; k<l1; k++)
    {
    floatT ti5=CC(0,2,k)+CC(0,2,k);
    floatT ti4=CC(0,4,k)+CC(0,4,k);
    floatT tr2=CC(ido-1,1,k)+CC(ido-1,1,k);
    floatT tr3=CC(ido-1,3,k)+CC(ido-1,3,k);
    CH(0,k,0)=CC(0,0,k)+tr2+tr3;
    floatT cr2=CC(0,0,k)+tr11*tr2+tr12*tr3;
    floatT cr3=CC(0,0,k)+tr12*tr2+tr11*tr3;
    floatT ci4, ci5;
  ...
  for (size_t k=0; k<l1;++k)
    for (size_t i=2; i<ido; i+=2)
      {
      size_t ic=ido-i;
      floatT tr2, tr3, tr4, tr5, ti2, ti3, ti4, ti5;
      ...
      floatT cr2=CC(i-1,0,k)+tr11*tr2+tr12*tr3;
      floatT ci2=CC(i  ,0,k)+tr11*ti2+tr12*ti3;
      floatT cr3=CC(i-1,0,k)+tr12*tr2+tr11*tr3;
      floatT ci3=CC(i  ,0,k)+tr12*ti2+tr11*ti3;
      floatT ci4, ci5, cr5, cr4;
      MULPM(cr5,cr4,tr5,tr4,ti11,ti12)
      MULPM(ci5,ci4,ti5,ti4,ti11,ti12)
      floatT dr2, dr3, dr4, dr5, di2, di3, di4, di5;
      PM(dr4,dr3,cr3,ci4)
      PM(di3,di4,ci3,cr4)
      PM(dr5,dr2,cr2,ci5)
      PM(di2,di5,ci2,cr5)
...
NOINLINE static void radbg(size_t ido, size_t ip, size_t l1,
  ...
  for (size_t l=1,lc=ip-1; l<ipph; ++l,--lc)
    ...
    for(; j<ipph; ++j,--jc)
      {
      iang+=l; if(iang>ip) iang-=ip;
      floatT war=csarr[2*iang], wai=csarr[2*iang+1];
      for (size_t ik=0; ik<idl1; ++ik)
        {
...
// All in CH
  ...
  for (size_t j=1; j<ip; ++j)
    ...
    for (size_t k=0; k<l1; ++k)
      ...
      for (size_t i=1; i<=ido-2; i+=2)
        {
        floatT t1=CH(i,k,j), t2=CH(i+1,k,j);
...
static void copy_and_norm(floatT *c, floatT *p1, size_t n, floatT fct)
  {
  if (p1!=c)
    {
    if (fct!=1.)
...
WARN_UNUSED_RESULT
static int rfftsp_forward(rfftsp_plan plan, floatT c[], floatT fct)
  {
  if (plan->length==1) return 0;
  size_t n=plan->length;
  size_t l1=n, nf=plan->nfct;
  floatT *ch = RALLOC(floatT, n);
  if (!ch) return -1;
  floatT *p1=c, *p2=ch;
...
WARN_UNUSED_RESULT
static int rfftsp_backward(rfftsp_plan plan, floatT c[], floatT fct)
  {
  if (plan->length==1) return 0;
  size_t n=plan->length;
  size_t l1=1, nf=plan->nfct;
...
WARN_UNUSED_RESULT
static int rfftsp_factorize (rfftsp_plan plan)
  {
  size_t length=plan->length;
  size_t nfct=0;
  while ((length%4)==0)
  ...
  size_t maxl=(size_t)(sqrt((floatT)length))+1;
  for (size_t divisor=3; (length>1)&&(divisor<maxl); divisor+=2)
    if ((length%divisor)==0)
      {
      while ((length%divisor)==0)
      ...
      maxl=(size_t)(sqrt((floatT)length))+1;
      }
  if (length>1) plan->fct[nfct++].fct=length;
  plan->nfct=nfct;
  return 0;
...
static size_t rfftsp_twsize(rfftsp_plan plan)
  {
  size_t twsize=0, l1=1;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
WARN_UNUSED_RESULT NOINLINE static int rfftsp_comp_twiddle (rfftsp_plan plan)
  {
  size_t length=plan->length;
  floatT *twid = RALLOC(floatT, 2*length);
  if (!twid) return -1;
...
NOINLINE static rfftsp_plan make_rfftsp_plan (size_t length)
  {
  if (length==0) return NULL;
  rfftsp_plan plan = RALLOC(rfftsp_plan_i,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  plan->mem=NULL;
  for (size_t i=0; i<NFCT; ++i)
    plan->fct[i]=(rfftsp_fctdata){0,0,0};
  if (length==1) return plan;
  if (rfftsp_factorize(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=rfftsp_twsize(plan);
  plan->mem=RALLOC(floatT,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (rfftsp_comp_twiddle(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }

NOINLINE static void destroy_rfftsp_plan (rfftsp_plan plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }

typedef struct fftsblue_plan_i
  {
  size_t n, n2;
  cfftsp_plan plan;
  floatT *mem;
...
typedef struct fftsblue_plan_i * fftsblue_plan;

NOINLINE static fftsblue_plan make_fftsblue_plan (size_t length)
  {
  fftsblue_plan plan = RALLOC(fftsblue_plan_i,1);
  if (!plan) return NULL;
  plan->n = length;
  plan->n2 = good_size(plan->n*2-1);
  plan->mem = RALLOC(floatT, 2*plan->n+2*plan->n2);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  plan->bk  = plan->mem;
  plan->bkf = plan->bk+2*plan->n;

/* initialize b_k */
  floatT *tmp = RALLOC(floatT,4*plan->n);
  ...
  for (size_t m=2*plan->n;m<=(2*plan->n2-2*plan->n+1);++m)
    plan->bkf[m]=0.;
  plan->plan=make_cfftsp_plan(plan->n2);
  if (!plan->plan)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  if (cfftsp_forward(plan->plan,plan->bkf,1.)!=0)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  DEALLOC(tmp);
...
NOINLINE static void destroy_fftsblue_plan (fftsblue_plan plan)
  {
  DEALLOC(plan->mem);
  destroy_cfftsp_plan(plan->plan);
  DEALLOC(plan);
  }

NOINLINE WARN_UNUSED_RESULT
static int fftsblue_ffts(fftsblue_plan plan, floatT c[], int isign, floatT fct)
  {
  size_t n=plan->n;
  size_t n2=plan->n2;
  floatT *bk  = plan->bk;
  floatT *bkf = plan->bkf;
  floatT *akf = RALLOC(floatT, 2*n2);
  if (!akf) return -1;

/* initialize a_k and ffts it */
  if (isign>0)
  ...
  if (cfftsp_forward (plan->plan,akf,fct)!=0)
...
/* do the convolution */
  if (isign>0)
    for (size_t m=0; m<2*n2; m+=2)
      {
      floatT im = -akf[m]*bkf[m+1] + akf[m+1]*bkf[m];
...
/* inverse ffts */
  if (cfftsp_backward (plan->plan,akf,1.)!=0)
    { DEALLOC(akf); return -1; }
...
WARN_UNUSED_RESULT
static int cfftsblue_backward(fftsblue_plan plan, floatT c[], floatT fct)
  { return fftsblue_ffts(plan,c,1,fct); }

WARN_UNUSED_RESULT
static int cfftsblue_forward(fftsblue_plan plan, floatT c[], floatT fct)
  { return fftsblue_ffts(plan,c,-1,fct); }

WARN_UNUSED_RESULT
static int rfftsblue_backward(fftsblue_plan plan, floatT c[], floatT fct)
  {
  size_t n=plan->n;
  floatT *tmp = RALLOC(floatT,2*n);
  if (!tmp) return -1;
  tmp[0]=c[0];
  tmp[1]=0.;
  memcpy (tmp+2,c+1, (n-1)*sizeof(floatT));
  if ((n&1)==0) tmp[n+1]=0.;
  ...
  if (fftsblue_ffts(plan,tmp,1,fct)!=0)
...
WARN_UNUSED_RESULT
static int rfftsblue_forward(fftsblue_plan plan, floatT c[], floatT fct)
  {
  size_t n=plan->n;
  floatT *tmp = RALLOC(floatT,2*n);
  if (!tmp) return -1;
  ...
  if (fftsblue_ffts(plan,tmp,-1,fct)!=0)
    { DEALLOC(tmp); return -1; }
  c[0] = tmp[0];
  memcpy (c+1, tmp+2, (n-1)*sizeof(floatT));
  DEALLOC(tmp);
  return 0;
  }

typedef struct cffts_plan_i
  {
  cfftsp_plan packplan;
  fftsblue_plan blueplan;
  } cffts_plan_i;

cffts_plan make_cffts_plan (size_t length)
  {
  if (length==0) return NULL;
  cffts_plan plan = RALLOC(cffts_plan_i,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_cfftsp_plan(length);
    if (!plan->packplan) { DEALLOC(plan); return NULL; }
    return plan;
    }
  floatT comp1 = cost_guess(length);
  floatT comp2 = 2*cost_guess(good_size(2*length-1));
  comp2*=1.5; /* fudge factor that appears to give good overall performance */
  if (comp2<comp1) // use Bluestein
    {
    plan->blueplan=make_fftsblue_plan(length);
...
void destroy_cffts_plan (cffts_plan plan)
  {
  if (plan->blueplan)
    destroy_fftsblue_plan(plan->blueplan);
  if (plan->packplan)
    destroy_cfftsp_plan(plan->packplan);
  DEALLOC(plan);
  }

WARN_UNUSED_RESULT int cffts_backward(cffts_plan plan, floatT c[], floatT fct)
  {
  if (plan->packplan)
    return cfftsp_backward(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftsblue_backward(plan->blueplan,c,fct);
  }

WARN_UNUSED_RESULT int cffts_forward(cffts_plan plan, floatT c[], floatT fct)
  {
  if (plan->packplan)
    return cfftsp_forward(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftsblue_forward(plan->blueplan,c,fct);
  }



typedef struct rffts_plan_i
  {
  rfftsp_plan packplan;
  fftsblue_plan blueplan;
  } rffts_plan_i;
...
rffts_plan make_rffts_plan (size_t length)
  {
  if (length==0) return NULL;
  rffts_plan plan = RALLOC(rffts_plan_i,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_rfftsp_plan(length);
    if (!plan->packplan) { DEALLOC(plan); return NULL; }
    return plan;
    }
  floatT comp1 = 0.5*cost_guess(length);
  floatT comp2 = 2*cost_guess(good_size(2*length-1));
  comp2*=1.5; /* fudge factor that appears to give good overall performance */
  if (comp2<comp1) // use Bluestein
    {
...
void destroy_rffts_plan (rffts_plan plan)
  ...
  if (plan->blueplan)
    destroy_fftsblue_plan(plan->blueplan);
  if (plan->packplan)
    destroy_rfftsp_plan(plan->packplan);
  DEALLOC(plan);
  }
...
WARN_UNUSED_RESULT int rffts_backward(rffts_plan plan, floatT c[], floatT fct)
  {
  if (plan->packplan)
    return rfftsp_backward(plan->packplan,c,fct);
  else // if (plan->blueplan)
    return rfftsblue_backward(plan->blueplan,c,fct);
  }

WARN_UNUSED_RESULT int rffts_forward(rffts_plan plan, floatT c[], floatT fct)
  {
  if (plan->packplan)
    return rfftsp_forward(plan->packplan,c,fct);
  else // if (plan->blueplan)
    return rfftsblue_forward(plan->blueplan,c,fct);
  }
